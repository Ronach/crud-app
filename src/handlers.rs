...
use sqlx::{PgPool, types::{chrono, uuid}};
...
pub async fn create_quote(extract::State(pool): extract::State<PgPool>, axum::Json(payload): axum::Json<CreateQuote>,
) -> Result<(http::StatusCode, axum::Json<Quote>), http::StatusCode> {
    let quote: Quote = Quote::new(payload.book, payload.quote);
